...
export class MessageCache extends BaseCache {
  ...
  public async addChatListToCache(
    ...
    try {
      ...
      console.log(`error from the addToChatListToCache..... ${error}`);
      log.error(error);
  ...
  public async addChatUsersToCache(value: IChatUsers): Promise<IChatUsers[]> {
    try {
      ...
      if (usersIndex === -1) {
        await this.client.RPUSH("chatUser", JSON.stringify(value));
  ...
  public async removeChatUsersFromCache(
    ...
    try {
      ...
      let chatUsers: IChatUsers[] = [];
      if (usersIndex > -1) {
        await this.client.LREM("chatUser", usersIndex, JSON.stringify(value));
        chatUsers = await this.getChatUsersList();
      } else {
        chatUsers = users;
      }
  ...
  public async getChatMessageFromCache(
    senderId: string,
    receiverId: string
  ...
  public async markMessagesAsDeleted(
    senderId: string,
    receiverId: string,
    messageId: string,
    type: string
  ...
  public async updateChatMessages(
    ...
    try {
      ...
      const messages: string[] = await this.client.LRANGE(
        `message:${parsedReceiver.conversationId}`,
        0,
        -1
      );
